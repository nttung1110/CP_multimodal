...
class VisualES():
    ...
    def initialize_model(self):
        self.face_detector = MTCNN(keep_all=False, post_process=False, min_face_size=40, device=self.args.device)
        self.emotion_recognizer = HSEmotionRecognizer(model_name=self.args.model_name,device=self.args.device)
    
    def extract_single_frame(self, img):
        # detect bounding boxes
        bounding_boxes, probs = self.face_detector.detect(img, landmarks=False)
        if bounding_boxes is not None:
            bounding_boxes = bounding_boxes[probs>self.threshold_face] # threshold_face = 0.6
    ...
    def find_face_tracks(self, video_path):
        ...
        while(cap.isOpened()):
            ...
            print("Processing frame: ", progress_frame)
            idx_frame += 1
