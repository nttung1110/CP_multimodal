...
#         pdb.set_trace()
#         # represent images as tensor first
#         for idx, value in tqdm(enumerate(frames)):
#             # test_2_frame.append(torch.Tensor(value).unsqueeze(dim=0).to('cuda'))
#             test_2_frame.append(value)

#             if idx % 100 == 0:
#                 print('Detecting bbox')
#                 bounding_boxes, probs = self.face_detector.detect(test_2_frame, landmarks=False)

#                 filter_boxes = []

#                 for box_each_frame in bounding_boxes:

#                     if box_each_frame is not None:
#                         box_each_frame = box_each_frame[probs>self.args.threshold_face] # threshold_face = 0.6

#                     filter_boxes.append(box_each_frame)

                


#                 # get face images from box
#                 [x1, y1, x2, y2] = box
...
#                 emotion, scores = self.emotion_recognizer.predict_multi_emotions(test_2_frame, logits=True)

#                 list_box_solve.extend([bounding_boxes])
#                 test_2_frame = []



#         # then input to face recognition model to get box

#         # interate through boxes for finding track, each track maintain tensor of facial images of that track

#         # iterate through track and extract emotional signals for that track
...
#     path_test_video = "/home/nttung/research/Monash_CCU/mini_eval/sub_data/converted_video/b26/M01003SS2.mp4"
